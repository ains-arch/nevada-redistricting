...
# results = pd.read_csv('data/cast_vote.csv') 
# print(f"DEBUG: results columns: {results.columns}")
# print(f"DEBUG: results: {results}")

# Clear csv if it exists, to avoid appending to old data
if os.path.exists("data/cast_vote_fixed.csv"):
    os.remove("data/cast_vote_fixed.csv")
...
# Open output file in write mode for the first chunk, then append
first_summary_chunk = True
...
for chunk in pd.read_csv('data/cast_vote.csv', header=4, chunksize=CHUNK_SIZE, low_memory=False):
    ...
    # Group by precinct and sum votes
    precinct_summary = chunk.groupby('precinct').agg({
        'harris': 'sum',
        'trump': 'sum',
        'other': 'sum'
    }).reset_index()
    ...
    precinct_summary['total_votes'] = (
        precinct_summary['harris'] + 
        precinct_summary['trump'] + 
        precinct_summary['other']
    )
    
    # If you want to append this summary to a separate CSV
    precinct_summary.to_csv('data/cast_vote_fixed.csv', 
                             mode='a', 
                             header=first_summary_chunk, 
                             index=False)
    first_summary_chunk = False

print(f"Total rows processed: {total_rows_processed}")
print(f"Total rows dropped: {total_rows_dropped}")
print(f"Processed data saved to data/cast_vote_fixed.csv")
